...
役割 : タスク組み立て＆タスク実行＆結果解析＆解読ヒント(LLMのためのメタ情報)の生成  

Layer-4 : Expression-Layer (表現層)  
役割：LLMによる応答文生成(意味を言語に変換、文脈に沿った自然な表現、人間に伝わる形に整える)  

Layer-5 : Validation-Layer（検証層）  
役割：出力検証＆RAGへのルール書き戻し(出力の安全性と整合性のチェック、新しい知見・洞察であるルールをRAGベースに書き戻す)
...
SUNIE-Core-Engine  
 ・SUNIE-InputAnalyzer(仮称)  
 ・SUNIE-OutputGenerater(仮称)  
 ・SUNIE-InputEncoder(仮称)  
 ・SUNIE-OutputDecoder(仮称)  
 ・SUNIE-TaskResolver(仮称)  
 ・SUNIE-PolicyChecker(仮称)  
